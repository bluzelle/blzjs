...
import {DEFAULT_TIMEOUT, sentryWithClient, defaultLease, defaultGasParams} from "../helpers/client-helpers";
...
const createKeys = (config: Config) => async (account: API): Promise<{account: API, time: number}> => {
    ...
    await account.withTransaction(() => times(config.NUMBER_OF_KEYS, n => account.create(`key-${n}`, 'x'.repeat(config.VALUE_LENGTH), defaultGasParams(), defaultLease)));
