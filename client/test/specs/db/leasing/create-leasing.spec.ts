...
import {defaultGasParams, defaultLease, zeroLease} from "../../../helpers/client-helpers";
...
describe('create', function () {
    ...
    ['days', 'hours', 'minutes', 'seconds'].forEach(async (unit) => {
        it(`should allow lease time in ${unit}`, async () => {
            await bz.create('myKey77', 'myValue',defaultGasParams(), {...defaultLease, [unit]: 20})
            expect(await bz.read('myKey77')).to.equal('myValue');
        })
    });

    it('should allow for lease time in multiple units', async () => {
        await bz.create('myKey44', 'myValue',defaultGasParams(), {...zeroLease, minutes: 30, hours: 1, days: 1, seconds: 30});
    ...
    it('should allow for a long lease period', async () => {
        await bz.create('myKey55', 'myValue', defaultGasParams(),{...defaultLease, days: 180});
    ...
    it('should not timeout a lease before the lease time expires', async () => {
        await bz.create('myShortKey2', 'myValue', defaultGasParams(), {...zeroLease, seconds: 15});
        await delay(9000);
