...
const bluzelle = (options) => monet_1.Some(options)
    .map(options => CommunicationService_1.newCommunicationService(options.url, options.mnemonic || ''))
    .map(cs => ({
    db: rpc_1.sdk(options, query_1.QueryClientImpl, tx_1.MsgClientImpl, MsgTypes, cs),
    nft: rpc_1.sdk(options, query_2.QueryClientImpl, tx_2.MsgClientImpl, MsgTypes, cs)
}))
    .join();
